...
export class EditorSession {
   ...
   * 本地长度为 0 才意味着"远端也没有块"。起始段使用与文档绑定的
   * 确定性 ID + 单槽 Y.Map（见 BlockDoc.ensureSeedParagraph）：两人几乎
   * 同时打开新文档时两端各插一次，CRDT 层也会收敛成同一个块，
   * 最终只保留一段，后打开的人看到先打开者那一段。
   */
  ...
  private seedIfEmpty(): void {
    ...
    if (this.blockDoc.length === 0) {
      const id = this.blockDoc.ensureSeedParagraph();
    ...
    binding.onEdit = (action) => {
      ...
      switch (action.type) {
        case 'insert':
          ...
          break;
        case 'replace':
          this.blockDoc.replaceText(id, action.index, action.length, action.text, {
            ...this.activeMarks,
          });
          this.setCaret({ blockId: id, index: action.index + action.text.length });
          break;

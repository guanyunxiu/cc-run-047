...
async function main(): Promise<void> {
  ...
  // 复刻浏览器端修复后的 seeding 规则：首轮同步后本地为空才插。
  // 起始段使用与文档绑定的确定性 ID + 单槽 Y.Map（ensureSeedParagraph），
  // 两端各自调用也在 CRDT 层收敛为同一个块。
  for (const c of [a, b]) {
    let seeded = false;
    c.onInitialSync = () => {
      if (!seeded) {
        seeded = true;
        if (c.blockDoc.length === 0) c.blockDoc.ensureSeedParagraph();
      }
    };
  }
  ...
  assert(
    a.blockDoc.getIds()[0] === b.blockDoc.getIds()[0] &&
      a.blockDoc.getIds()[0] === SEED_BLOCK_ID,
    '两侧段落是同一个 CRDT 块（与文档绑定的确定性起始段）',
  );

...
export class BlockRenderer {
  ...
  private render(event?: DocChangeEvent): void {
    ...
    if (this.pendingFocus) {
      ...
      if (mounted) {
        // 焦点已经在本块、光标也已在目标位置时（普通打字 / 选区移动会不断
        // 回调 focusBlock），不要 removeAllRanges + focus：这会重置浏览器
        // 选区并打断系统输入法的组字。只有跨块 / 块刚挂载时才真正重设。
        if (!this.caretAlreadyAt(mounted.content, index)) {
  ...
  /** 当前 DOM 选区是否已是 content 内 index 处的折叠光标。 */
  private caretAlreadyAt(content: HTMLElement, index: number): boolean {
    ...
    if (!node || (!content.contains(node) && node !== content)) return false;
    const range = document.createRange();
    range.selectNodeContents(content);
    range.setEnd(node, sel.anchorOffset);
    return range.toString().length === index;
  }
